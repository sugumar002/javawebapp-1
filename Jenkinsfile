...
environment {
    ...
    steps {
	script {
	  def lastet_version= sh(script: "curl -u ${USERNAME}:${PASSWORD} -s ${NEXUS_URL}/service/rest/v1/search/assets?repository=${REPOSITORY}&group=${GROUP_ID}&name=${ARTIFACT_ID}&sort=version&format=maven2&maven.extension=${PACKAGING} | jq -r '.items[0].version'", returnStdout: true).trim()
